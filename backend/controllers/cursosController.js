...
const createCurso = async (req, res) => {
  ...
  try {
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    const values = [titulo, descripcion, imagen_url, codigo_acceso, id_categoria, precio, estado || 'borrador'];
...
const updateCurso = async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE id_curso = ?
    `;
    const values = [titulo, descripcion, imagen_url, codigo_acceso, id_categoria, precio, estado, id_curso];
    await db.promise().query(query, values);
