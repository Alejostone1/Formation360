...
export default function PurchaseInterestForm({ course }: { course: Course }) {
    ...
    useEffect(() => {
        if (isDialogOpen) {
            // Check for logged-in user in localStorage on the client side when dialog opens
            const loggedInUser = localStorage.getItem('user');
            if (loggedInUser) {
                const parsedUser = JSON.parse(loggedInUser);
                setUser(parsedUser);
                setFormData({ name: parsedUser.nombre_completo, email: parsedUser.email, city: parsedUser.ciudad || '' });
            }
        }
    }, [isDialogOpen]);
    ...
    const handleSendWhatsApp = () => {
        ...
        const message = `¡Hola! 👋 Estoy interesado/a en el curso "${courseName}". Mi nombre es ${userName}, mi correo es ${userEmail}${userCity ? ` y vivo en ${userCity}` : ''}. ¡Quisiera más información, por favor!`;
